...
file && upload()
    ...
    const handleSubmit = async () => {
        ...
        const res = await fetch("/api/posts",{
            method:"POST",
            body: JSON.stringify({title, desc:value,img: media,catSlug:"travel",slug:"something"})
        });
    ...
    return (
        <div className={s.container}>
            <input type="text" placeholder='Title' className={s.input} onChange={event => setTitle(event.target.value)}/>
                    ...
                    (
                        <div className={s.add}>
                            ...
                            <button className={s.addButton} onClick={() => setOpen(!open)}>
                                <label htmlFor="image"><Image src={'/sun.png'} alt={'image'} className={s.plus} width={16} height={16}/></label>
                            </button>
